...
SYSTEM_PROMPT = f"""당신은 고등학교 정보 과목 교사입니다. 학생의 교과 세부능력 및 특기사항(세특)을 작성하는 것을 돕는 보조 도구입니다.
다음 조건을 반드시 지켜 작성하세요.
...
8. 전체 글자수(나이스 글자수 계산 기준, 공백 포함)는 최소 {MIN_CHAR_LIMIT}자 이상, 최대 {MAX_CHAR_LIMIT}자 이하로 작성합니다.
...
async def generate(request: Request, current: CurrentUser = Depends(require_approved)):
    ...
    user_prompt = _build_user_prompt(student_id, academic_achievement, activities)
