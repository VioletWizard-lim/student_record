...
- **화면 구성**: 로그인하면 왼쪽에 사이드바가 표시되며, "생기부 문구 생성"(`/dashboard`), "생성 이력"(`/history`), "계정 관리"(`/account`, 담당 과목 수정 + 계정 삭제, 관리자 제외), "관리자 모드"(`/admin`, 관리자만)로 이동할 수 있습니다. 승인 대기 중에는 생성/이력 메뉴가 숨겨지고 계정 관리만 노출됩니다.
