...
- **활동 개수**: 활동은 1개로 고정되지 않고 화면에서 자유롭게 추가/삭제할 수 있습니다(최대 10개, `app/generation.py`의 `MAX_ACTIVITIES`에서 조정 가능). 각 활동마다 성취기준(A~E)과 관찰 자료를 입력합니다.
- **생성 결과**: 입력한 교과 성취 수준과 활동들을 모두 종합해, "교과 성취 수준 → 수행 특기사항 → 교과 역량 → 수업 태도" 순서를 따르는 **하나의 세특 문단**으로 생성합니다(활동별로 나누지 않음). 글자수는 최소 600자, 최대 700자를 목표로 하며, `app/generation.py`의 `MIN_CHAR_LIMIT`/`MAX_CHAR_LIMIT`에서 조정 가능합니다.
- **글자수 계산**: 생성 결과의 글자수는 나이스(NEIS) 공식 계산식(`=2*LENB(cell)-LEN(SUBSTITUTE(cell,CHAR(10),""))`)을 그대로 이식한 `app/charcount.py`의 `neis_char_count()`로 계산해 화면에 표시합니다. 목표 범위를 벗어나면 결과 화면에 경고가 표시됩니다.
