...
- **생성 이력 검색/정렬/페이지네이션**: `/history`에서 학번·영역·결과 내용을 대상으로 검색할 수 있고(대소문자 구분 없음), "일시"/"학번"/"영역" 열 제목을 클릭하면 해당 기준으로 오름차순/내림차순 정렬됩니다(다시 클릭하면 순서가 뒤집힘). "페이지당 표시"에서 10/20/100명씩 보기를 선택할 수 있고, 하단에서 페이지를 넘길 수 있습니다(`app/generation.py`의 `HISTORY_PAGE_SIZES`에서 조정 가능). 검색/정렬/페이지네이션은 최근 500건(`app/generation.py`의 `HISTORY_FETCH_LIMIT`) 범위 내에서 서버가 처리합니다.
