...
- **활동 개수**: 활동은 1개로 고정되지 않고 화면에서 자유롭게 추가/삭제할 수 있습니다(최대 10개, `app/generation.py`의 `MAX_ACTIVITIES`에서 조정 가능). 각 활동마다 성취기준(A~E)과 관찰 자료를 입력하면, 입력한 개수와 동일한 개수의 결과를 생성합니다.
- **글자수 계산**: 생성 결과(모든 활동 합산)의 글자수는 나이스(NEIS) 공식 계산식(`=2*LENB(cell)-LEN(SUBSTITUTE(cell,CHAR(10),""))`)을 그대로 이식한 `app/charcount.py`의 `neis_char_count()`로 계산해 화면에 표시합니다. 목표 글자수(기본 1000자)를 넘으면 결과 화면에 경고가 표시됩니다.
