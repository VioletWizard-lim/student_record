...
- **교사별 담당 과목**: 회원가입 화면에서 그때까지 등록된 과목(`SUBJECT_CRITERIA`의 전체 목록) 중 본인이 담당하는 과목을 복수 선택할 수 있고, `/account`(계정 설정)에서 나중에 언제든 바꿀 수 있습니다. 대시보드의 "과목" 선택 상자에는 본인이 등록한 과목만 나타나 다른 교사의 과목과 섞이지 않습니다. 아무 과목도 등록하지 않은 계정(과거 가입자 포함)은 하위 호환을 위해 전체 과목이 대신 표시됩니다. `/generate` 서버 검증에서도 본인이 등록한 과목 외의 값은 거부합니다.
...
- **화면 구성**: 로그인하면 왼쪽에 사이드바가 표시되며, "생기부 문구 생성"(`/dashboard`), "생성 이력"(`/history`), "계정 설정"(`/account`, 담당 과목 수정 + 계정 삭제, 관리자 제외), "관리자 모드"(`/admin`, 관리자만)로 이동할 수 있습니다. 승인 대기 중에는 생성/이력 메뉴가 숨겨지고 계정 설정만 노출됩니다.
