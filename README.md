...
- **글자수 계산**: 생성 결과(활동1+활동2 합산)의 글자수는 나이스(NEIS) 공식 계산식(`=2*LENB(cell)-LEN(SUBSTITUTE(cell,CHAR(10),""))`)을 그대로 이식한 `app/charcount.py`의 `neis_char_count()`로 계산해 화면에 표시합니다. 목표 글자수(기본 1000자)를 넘으면 결과 화면에 경고가 표시됩니다.
