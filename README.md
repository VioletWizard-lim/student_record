...
- **바이트**: 목표 바이트(최소/최대)는 화면에서 교사가 직접 지정합니다(기본값 600~700바이트). 최대 바이트는 2500바이트를 넘을 수 없도록 서버에서 강제합니다(`app/generation.py`의 `HARD_MAX_CHAR_LIMIT`). 바이트 수는 나이스(NEIS) 공식 계산식(`=2*LENB(cell)-LEN(SUBSTITUTE(cell,CHAR(10),""))`)을 그대로 이식한 `app/charcount.py`의 `neis_byte_count()`로 계산해 화면에 표시합니다.
